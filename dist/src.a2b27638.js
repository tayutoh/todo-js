...
var onClickAdd = function onClickAdd() {
  ...
  document.getElementById("add-text").value = ""; // 追加する要素の生成
...
var createIncompleteList = function createIncompleteList(text) {
  // li生成
  var createLi = document.createElement("li"); // list-row生成

  var createListRow = document.createElement("div");
  createListRow.className = "list-row"; // list-text生成

  var createListText = document.createElement("p");
  createListText.innerText = text;
  createListText.className = "list-text"; // 完了ボタン生成

  var COMPLETE_TEXT = "完了";
  var createCompleteButton = document.createElement("button");
  createCompleteButton.innerText = COMPLETE_TEXT;
  createCompleteButton.addEventListener("click", function () {
    // 押下された完了ボタンの祖先要素li取得
    var completeTarget = createCompleteButton.closest("li"); // 押下された完了ボタンの親liを未完了リストから削除
    ...
    document.getElementById("complete-list").appendChild(completeTarget); // 押下された完了ボタンの親要素list-row取得

    var completeParentTarget = createCompleteButton.parentNode; // 戻すボタン生成

    var RETURN_TEXT = "戻す";
    var createUndoButton = document.createElement("button");
    createUndoButton.innerText = RETURN_TEXT;
    createUndoButton.addEventListener("click", function () {
      // 押下された戻るボタンの親liを完了リストから削除
      var deleteTarget = createUndoButton.closest("li");
      document.getElementById("complete-list").removeChild(deleteTarget); // TODOのテキスト取得

      var todoText = createUndoButton.parentNode.firstElementChild.innerText;
      createIncompleteList(todoText);
    });
    completeParentTarget.appendChild(createUndoButton); // 押下された完了ボタンの次にある削除ボタンを削除

    createCompleteButton.nextSibling.remove(); // 押下された完了ボタンを削除
  ...
  createDeleteButton.addEventListener("click", function () {
    ...
    var deleteTarget = createDeleteButton.closest("li");
  ...
  createListRow.appendChild(createDeleteButton); // 未完了リストに追加
